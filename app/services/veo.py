...
Nội dung / kịch bản video và giọng đọc sẽ bổ sung sau; hiện dùng các kiểu chuyển động có sẵn (VEO_STYLES).
"""
import time
...
# Kiểu chuyển động (prompt tiếng Anh cho Veo hiểu tốt nhất). Nội dung chi tiết sẽ tuỳ chỉnh sau.
VEO_STYLES = {
...
def build_prompt(product: dict, style: str = "studio") -> str:
    ...
    motion = VEO_STYLES.get(style, VEO_STYLES["studio"])[1]
...
def generate(frame_path: str, prompt: str, out_path: str, model: str | None = None,
             poll_seconds: int = 10, timeout: int = 900) -> dict:
